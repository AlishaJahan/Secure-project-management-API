...
export const viewProject = async (req, res, next) => {
    try {
        ...
        // Parallelize fetching projects and the total count for pagination
...
export const unassignUserFromProject = async (req, res, next) => {
    try {
        ...
        // Process all user unassignments in parallel
        await Promise.all(userIdsToUnassign.map(async (userIdToUnassign) => {
            ...
            // Both DELETE and UPDATE are independent for this user, so we can parallelize them too if we want
            await Promise.all([
